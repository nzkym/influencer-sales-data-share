...
1. 📊 시장 전반 현황 요약
   - 5개 대분류(식품·생활건강·출산육아·화장품미용·디지털가전)에서 나타나는 건강 소비 트렌드의 특징
   - 카테고리를 넘나드는 공통 트렌드 키워드 또는 주목할 거시적 흐름 (2-3가지)

2. ⚡ 초반 선점 유력 키워드 (우선순위 높음)
   - 아직 경쟁이 치열하지 않지만 빠르게 성장 중인 키워드
   - 어느 카테고리에서 나온 키워드인지, 왜 지금이 선점 기회인지 설명
   - 약사 전문성으로 접근 가능한 제품화 아이디어 포함

3. 📈 안정 성장 키워드 (꾸준한 우상향)
   - 장기적으로 꾸준히 성장 중인 키워드 (수요가 검증됨)
   - 진입 시 경쟁 강도와 성장 지속 가능성 평가
   - 중장기 진입 전략 제안

4. 🔍 카테고리별 주목 키워드
   - 식품 전체: 건강 연관 식품 트렌드 (레몬즙·헛개나무분말·가르시니아 등 사례 포함)
   - 생활/건강: 건강관리·반려동물 등 신규 시장 기회
   - 출산/육아: 어린이 건강식품·이유식·유아 위생 관련 트렌드
   - 화장품/미용: 약국 화장품 연계 가능 카테고리
   - 디지털/가전: 이미용가전·홈 헬스케어 기기 트렌드
