...
1. 실버산업 / 고령친화용품 (의료용품, 재활운동용품, 안마용품, 건강측정용품, 당뇨관리용품 등) — 최우선
...
반드시 아래 JSON 형식으로만 답하세요. 관련 있는 키워드만 배열에 담으세요 (후보 목록에 있는 표기 그대로):
{{"relevant": ["키워드1", "키워드2", ...]}}"""


def _classify_batch(keywords: list[str], api_key: str) -> list[str]:
    """
    한 배치를 분류. 실패 시(파싱 오류 등) 배치 전체를 그대로 반환한다 —
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            return [kw for kw in relevant if kw in candidate_set]
...
def filter_relevant_keywords(keyword_candidates: list[dict]) -> list[dict]:
    ...
    Returns: 관련 있다고 판단된 항목만 남긴 리스트 (원래 dict 그대로, 순서 유지)
    """
    ...
    for idx, batch in enumerate(batches):
        ...
        relevant = _classify_batch(batch, api_key)
        relevant_set.update(relevant)
        print(f"[필터] 배치 {idx + 1}: {len(relevant)}/{len(batch)}개 관련 있음")

    filtered = [c for c in keyword_candidates if c["keyword"] in relevant_set]
    print(
        f"\n[필터] 완료: {len(keyword_candidates)}개 → {len(filtered)}개 관련 키워드 선별 "
        f"(제외 {len(keyword_candidates) - len(filtered)}개)"
    )
    return filtered
